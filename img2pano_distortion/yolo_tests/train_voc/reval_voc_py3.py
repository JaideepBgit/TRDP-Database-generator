...
def get_voc_results_file_template(image_set, out_dir = 'results'):
    ...
    path = os.path.join(out_dir, filename) # pcl changed to allow different folders tha results
...
def do_python_eval(devkit_path, year, image_set, classes, output_dir = 'results'):
    annopath = os.path.join(
        devkit_path,
        #'VOC' + year, removed to work with Yshift dir structures
        'Annotations',
        '{}.xml')
    ...
    print('--------------------------------------------------------------')
    # pcl: print aps in xls file. Still not clear why does not write decimal separator.??
    csv_filename = os.path.split(output_dir)[0] + '/csv_results/' + os.path.split(output_dir)[1]+ '.csv'
    aps_str = [str('{:.5f}'.format(ap)) for ap in aps]
